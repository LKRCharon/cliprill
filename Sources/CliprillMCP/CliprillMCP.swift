...
enum CliprillMCP {
    ...
    private static var specs: [ToolSpec] {
        ...
        return [
            ...
            spec("queue_remove", "Remove one remaining queue item; history is preserved. Empty queues are deleted by default (deleted: true); Settings can disable this.", write.merging(["item_id": string]) { $1 }, ["queue_id", "item_id", "expected_revision", "idempotency_key"]),
            ...
            spec("queue_undo_last", "Restore the last dispatched item as next and pause. Requires the queue to still exist; empty queues are deleted by default. Does not undo text in the target app.", write, ["queue_id", "idempotency_key"]),
    ...
    static func main() async {
        do {
            ...
            let server = Server(name: "Cliprill", version: "0.7.0", capabilities: .init(tools: .init(listChanged: false)))
