...
当前版本：**0.7.0**。来源默认隐藏，单击延迟预览、粘贴置顶均可设置。
...
安全输入不再阻止历史／常用内容的直接粘贴尝试。目标应用若不接受模拟粘贴，可使用右键「复制到剪贴板」，再自行按 ⌘V；列表聚焦时也支持 ⌘C。队列自动出队仍需全局键盘监听。
...
- 历史数量、保留天数，以及不记录的应用。
- 队列为空时是否自动删除（默认开启）。
- 默认将粘贴过的历史条目置顶，可关闭；自动预览默认等待 0.5 秒，可调整。
